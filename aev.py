...
def angular_terms(Rca: float, ShfZ: Tensor, EtaA: Tensor, Zeta: Tensor,
    ...
    factor1 = torch.exp(- 8.0 * (angles - ShfZ)**2)
...
def compute_aev(species: Tensor, coordinates: Tensor, triu_index: Tensor,
    ...
    even_closer_indices = torch.nonzero(distances <= Rca, as_tuple=False).flatten()
...
def compute_radial_qaev(
    species: Tensor, coordinates: Tensor, charges: Tensor,
    constants: Tuple[float, Tensor, Tensor, float, Tensor, Tensor, Tensor, Tensor],
    ...
    radial_aev.index_add_(0, index12[0], radial_terms_ * charges12[1].unsqueeze(-1))
    radial_aev.index_add_(0, index12[1], radial_terms_ * charges12[0].unsqueeze(-1))
    radial_aev = radial_aev.reshape(num_molecules, num_atoms, radial_length)
    return radial_aev
