...
sys.path.append('../')
try:
    from nn import SharedSANIModel, SANIModel, EMA, ActivationHistogram
...
training = anidata_loader(data_path + '*train_*', ('charges',)).shuffle()
validation = anidata_loader(data_path + '*valid_*', ('charges',)).shuffle()

training = training.cache()
validation = validation.collate(batch_size).cache()

###############################################################################
model = SharedSANIModel(aev_computer, network_dims, out_dim=10).to(device)
print(model)
...
def validate():
    with torch.no_grad():
        ...
        for properties in validation:
            ...
            _, predicted_energies = model((species, coordinates, net_charges))
...
for epoch in range(max_epochs):
    ...
    rmse = validate()
    training_writer.add_scalar('ema_vrmse', ema_rmse, epoch)
    training_writer.add_scalar('vrmse', rmse, epoch)
            ...
            for name, param in model.named_parameters():
                ...
                if param.grad is not None:
                    ...
                    training_writer.add_scalar(
                        str(name) + "_grad_stddev", torch.std(param.grad.data, unbiased=False).cpu().numpy(), iteration
                    )
